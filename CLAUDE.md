...
- **Painel:** busca full-text **SQLite FTS5/BM25** (acento-insensível, `backend/search.py`), filtros (fonte/categoria/secretaria/prazo/valor — o de secretaria só aparece quando há oportunidades curadas), **ciclo de vida** (vencidas ocultas por padrão, ordenação por urgência, selos nova/vencida), página de detalhe `/opportunity/{id}`, e **matching municipal** (aderência 0–100, `processing/matching.py`, penaliza prazo vencido). Quando a curadoria LLM rodou, a lista mostra o resumo curado (✦) no lugar do snippet da descrição e a secretaria sugerida como link-filtro 🏛️.
